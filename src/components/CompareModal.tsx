                    ...
                    return (
                      <td key={p.id} className="p-3 text-center">
                        <button 
                          ...
                          className={`w-full max-w-[150px] mx-auto py-1.5 rounded-lg font-bold text-[10px] sm:text-xs flex items-center justify-center gap-1 flex-row flex-nowrap whitespace-nowrap cursor-pointer transition-colors ${isOutOfStock ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : addedProductId === p.id ? 'bg-green text-white' : 'bg-plum text-white hover:bg-plum-dark'}`}
                        >
                          {addedProductId === p.id ? (
                            <>
