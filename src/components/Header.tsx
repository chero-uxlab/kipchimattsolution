...
export default function Header({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const selectEl = document.querySelector('.goog-te-combo') as HTMLSelectElement;
      if (selectEl) {
        const targetVal = selectedLang === 'en' ? '' : selectedLang;
  ...
  return (
    <>
      ...
      <header className="bg-plum text-white py-2 shadow-md border-b border-plum-dark font-sans w-full">
        <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-3 lg:gap-5">
          ...
          <div className="hidden md:flex items-center gap-1 lg:gap-2.5 xl:gap-3.5 shrink-0" id="header-right-actions">
            ...
            <div 
              className="hidden md:flex relative items-center rounded-sm border border-transparent hover:border-white px-2 py-1 cursor-pointer shrink-0 transition-all select-none"
            ...
            >
              <div className="flex items-center gap-1">
                ...
                <ChevronDown className="w-2.5 h-2.5 text-gray-350" />
              </div>

              {/* Amazon-Style Custom Dropdown Menu Popover */}
              {langDropdownOpen && (
                <div 
                  className="absolute top-full right-0 mt-1 bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-md shadow-2xl py-3 px-4 w-60 text-gray-800 dark:text-gray-100 z-50 animate-scale-up border-t-4 border-t-orange cursor-default" 
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Speech Bubble Arrow pointing up */}
                  <div className="absolute right-6 -top-2 w-3 h-3 bg-white dark:bg-gray-950 border-t border-l border-gray-200 dark:border-gray-800 transform rotate-45 z-10" />
                  
                  <div className="space-y-3 relative z-20">
                    <div className="flex items-center justify-between pb-1 border-b border-gray-100 dark:border-gray-850">
                      <span className="text-[10px] text-gray-400 dark:text-gray-500 font-extrabold uppercase tracking-wider">Change Language</span>
                      <span className="text-[9px] text-orange font-bold hover:underline cursor-pointer">Learn more</span>
                    </div>
                      ...
                      {languages.map((lang) => {
                        ...
                        return (
                          <div 
                            ...
                            className="flex items-center gap-2.5 cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-900/50 p-1 rounded transition-colors"
                          >
                            <div className="relative flex items-center justify-center shrink-0">
                              <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all ${
                                isSelected 
                                  ? 'border-orange bg-white dark:bg-gray-950' 
                                  : 'border-gray-300 dark:border-gray-750 group-hover:border-orange'
                              }`}>
                                {isSelected && (
                            ...
                            <span className="text-[11px] font-bold flex items-center gap-1.5 text-gray-700 dark:text-gray-200 group-hover:text-gray-950 dark:group-hover:text-white">
                              <span>{lang.flag}</span>
                              <span>{lang.label}</span>
                              <span className="text-[9px] text-gray-400 font-semibold uppercase">-{lang.code}</span>
                            </span>
                          </div>
                        );
                      })}
                    ...
                    <div className="h-px bg-gray-150 dark:bg-gray-850 my-2" />

                    {/* Currency / Regional Info */}
                    <div className="space-y-1.5">
                      <span className="text-[10px] text-gray-400 dark:text-gray-500 font-extrabold uppercase tracking-wider block">Change Currency</span>
                      <div className="flex items-center justify-between text-[11px] font-bold text-gray-750 dark:text-gray-200 p-1.5 bg-gray-50 dark:bg-gray-900 rounded">
                        <span className="flex items-center gap-1">
                          <span className="text-orange font-black">KES</span>
                          <span>Kenyan Shilling</span>
                        </span>
                        <span className="text-[9px] text-gray-400 font-extrabold uppercase">Default</span>
                      </div>
                    </div>
                    ...
                    {/* Shopping on message */}
                    <div className="text-center bg-orange/5 dark:bg-orange/10 border border-orange/10 rounded p-2 text-[9px] text-gray-500 dark:text-gray-400 font-semibold leading-relaxed">
                      You are shopping on <strong className="text-orange font-bold">Kipchimatt.ke</strong>
                    </div>
                  </div>
                </div>
              )}
            ...
            {currentView === 'shop' && (
              <button 
                onClick={onToggleCart}
                className="flex items-center gap-1.5 px-1.5 lg:px-2 py-1 rounded-sm border border-transparent hover:border-white cursor-pointer relative shrink-0 select-none group transition-all flex-row flex-nowrap whitespace-nowrap"
                aria-label="Shopping Cart"
              ...
              >
                ...
                <span className="text-[11px] lg:text-xs text-white font-black uppercase tracking-wider hidden lg:inline shrink-0">
                  Cart
                </span>
              </button>
            )}
      ...
      {currentView === 'shop' && (
        <nav className="bg-plum-fade/60 dark:bg-pink-950/15 border-b border-plum/15 dark:border-pink-950/30 shadow-sm overflow-hidden select-none" id="quick-category-rail">
            ...
            {categoryMeta.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-4.5 py-1.5 rounded-full font-extrabold text-xs whitespace-nowrap transition-all cursor-pointer border active:scale-95 shadow-sm hover:shadow-md ${
                    isActive 
                      ? 'bg-plum text-white border-plum hover:bg-plum-light dark:bg-plum-light dark:border-plum-light dark:text-white shadow-md shadow-plum/30 scale-105 z-10' 
                      : 'bg-white dark:bg-gray-850 text-plum-dark dark:text-pink-100 border-plum/30 dark:border-pink-800/40 hover:border-plum dark:hover:border-plum-light hover:bg-plum/5'
                  }`}
                >
                  {cat.label}
                </button>
          ...
          <nav className="fixed top-0 left-0 w-[300px] max-w-[85%] h-full bg-white dark:bg-gray-900 shadow-2xl z-55 flex flex-col font-sans">
            ...
            <div className="flex-1 overflow-y-auto py-3 flex flex-col font-bold text-sm text-gray-700 dark:text-gray-300">
              ...
              <div className="px-5 py-3 mt-auto">
                <span className="text-[10px] text-gray-400 font-extrabold uppercase tracking-widest block mb-1.5">Translation Language</span>
                <div className="relative">
                  <select
                    value={selectedLang}
                    onChange={(e) => {
                    ...
                    className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-800 rounded-xl px-4 py-2 text-xs text-gray-850 dark:text-gray-100 font-extrabold outline-none appearance-none cursor-pointer"
