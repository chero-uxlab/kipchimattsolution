...
export default function AdminPortal({
  ...
  // --- Login & User Management States ---
  const [usersList, setUsersList] = useState<any[]>([]);
  const [newUserUsername, setNewUserUsername] = useState('');
  const [newUserPassword, setNewUserPassword] = useState('');
  const [newUserRole, setNewUserRole] = useState<'superadmin' | 'admin' | 'staff'>('staff');
    ...
    try {
      const res = await fetch('/api/auth/users', {
        ...
        body: JSON.stringify({
          ...
          email: newUserEmail.trim() || `${newUserUsername}@kipchimatt.co.ke`
        })
      });
      if (res.ok) {
        onShowToast(`User account for "${newUserUsername}" created successfully.`, 'success');
        setNewUserUsername('');
        setNewUserPassword('');
        setNewUserName('');
        setNewUserEmail('');
        ...
        onShowToast(err.error || 'Failed to create user account.', 'error');
      }
    } catch (err) {
      console.error(err);
      onShowToast('Connection error adding user account.', 'error');
    }
  };

  const handleDeleteUser = async (id: string) => {
    // If we can match username/id of the loggedInUser
    if (id === loggedInUser?.id || usersList.find(u => u.id === id)?.username === loggedInUser?.username) {
      onShowToast('Forbidden: Cannot delete your own active session account.', 'error');
      return;
    }
    if (!window.confirm('Are you absolutely certain you want to terminate this admin user account?')) {
      return;
    }
    try {
      ...
      if (res.ok) {
        onShowToast('User account successfully terminated.', 'success');
        fetchUsers();
      } else {
        const err = await res.json();
        onShowToast(err.error || 'Failed to delete user account.', 'error');
      }
    } catch (err) {
      console.error(err);
      onShowToast('Connection error deleting user account.', 'error');
    }
  };

  React.useEffect(() => {
    if (isLoggedIn && loggedInUser?.role === 'superadmin' && activePage === 'users') {
      fetchUsers();
    }
  }, [isLoggedIn, loggedInUser, activePage]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.user) {
          onLogin(data.user);
          onShowToast(`Welcome back, ${data.user.name || data.user.username}! Access granted.`, 'success');
        } else {
          onShowToast('Authentication failed.', 'error');
        }
      } else {
        const errData = await res.json().catch(() => ({}));
        onShowToast(errData.error || 'Invalid username or password.', 'error');
      }
    } catch (err) {
      console.error("Login failed:", err);
      onShowToast('Could not connect to authentication service.', 'error');
    }
  };
    ...
    return (
      <div className="min-h-[85vh] flex items-center justify-center py-12 px-4 bg-gradient-to-br from-plum to-plum-dark rounded-2xl my-4 max-w-7xl mx-auto shadow-2xl">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 border border-white/10 select-none">
          ...
          <div className="mt-8 text-center bg-gray-50 border border-gray-100 rounded-lg p-3.5 space-y-1.5 text-left">
            <p className="text-[10px] text-gray-400 font-black uppercase tracking-wider text-center mb-1">DEMO ROLES & ACCESS RIGHTS</p>
            <div className="text-xs text-gray-600 font-bold border-b border-gray-100 pb-1">
              👑 <span className="text-red-700">Superadmin:</span> <span className="font-mono text-plum underline">superadmin</span> / <span className="font-mono text-plum underline">super123</span>
              <p className="text-[10px] text-gray-400 font-normal">Full privileges, users management & settings.</p>
            </div>
            <div className="text-xs text-gray-600 font-bold border-b border-gray-100 pb-1">
              💼 <span className="text-plum">Admin:</span> <span className="font-mono text-plum underline">admin</span> / <span className="font-mono text-plum underline">admin123</span>
              <p className="text-[10px] text-gray-400 font-normal">Manage catalog & orders. No settings or user CRUD.</p>
            </div>
            <div className="text-xs text-gray-600 font-bold">
              🛠️ <span className="text-gray-600">Staff:</span> <span className="font-mono text-plum underline">staff</span> / <span className="font-mono text-plum underline">staff123</span>
              <p className="text-[10px] text-gray-400 font-normal">Read-only monitoring dashboard & reports.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col lg:flex-row gap-6">
      ...
      <aside className="w-full lg:w-64 bg-white border border-gray-150 rounded-2xl p-5 shadow-sm h-fit">
        <div className="flex flex-col border-b border-gray-100 pb-4 mb-4 select-none">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-plum text-white flex items-center justify-center font-black text-sm shadow-sm uppercase">
              {(loggedInUser?.name || loggedInUser?.username || 'OP')[0]}
            </div>
            <div className="min-w-0 flex-1">
              <h4 className="font-extrabold text-gray-800 text-sm leading-tight truncate">
                {loggedInUser?.name || loggedInUser?.username || 'System Operator'}
              </h4>
              <p className="text-[10px] text-gray-400 truncate mt-0.5">{loggedInUser?.email || 'operator@kipchimatt.co.ke'}</p>
            </div>
          </div>
          <div className="mt-3">
            <span className={`inline-block text-[9px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full ${
              loggedInUser?.role === 'superadmin' 
                ...
                ? 'bg-plum-fade text-plum border border-plum/15' 
                : 'bg-gray-100 text-gray-500 border border-gray-200'
            }`}>
              🛡️ {loggedInUser?.role || 'Staff'}
            </span>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-1">
          ...
          {loggedInUser?.role === 'superadmin' && (
            <button 
              ...
              className={`flex items-center gap-3 w-full text-left px-4 py-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${activePage === 'users' ? 'bg-plum text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              <ShieldCheck className="w-4 h-4 text-red-600" />
              <span>Admin Accounts</span>
              <span className="ml-auto text-[9px] bg-red-100 text-red-700 font-extrabold px-1.5 py-0.5 rounded uppercase">
                SECURE
              </span>
            </button>
          )}
      ...
      <main className="flex-1 space-y-6">
        ...
        <div className="flex justify-between items-center border-b border-gray-150 pb-3">
          <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight">
            ...
            {activePage === 'users' && 'Admin Accounts Control'}
          </h2>
        ...
        {activePage === 'products' && (
          <div className="space-y-4">
            ...
            {loggedInUser?.role === 'staff' && (
              <div className="bg-plum-fade border border-plum/10 rounded-lg p-3 text-plum flex items-center gap-2 text-xs select-none">
                <ShieldCheck className="w-4.5 h-4.5 text-plum" />
                <span>Logged in as <strong>Operations Staff</strong>. Catalog inventory levels are read-only.</span>
              </div>
            )}
                        ...
                        <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                          ...
                          <td className="p-4 text-right space-x-1 whitespace-nowrap">
                            {loggedInUser?.role !== 'staff' ? (
                              ...
                              <span className="text-[10px] bg-gray-50 text-gray-400 font-bold px-2 py-1 rounded select-none inline-flex items-center gap-1 border border-gray-200">
        ...
        {activePage === 'orders' && (
          <div className="space-y-4">
            ...
            {loggedInUser?.role === 'staff' && (
              <div className="bg-plum-fade border border-plum/10 rounded-lg p-3 text-plum flex items-center gap-2 text-xs select-none">
                <ShieldCheck className="w-4.5 h-4.5 text-plum" />
                <span>Logged in as <strong>Operations Staff</strong>. Orders records are read-only. Status updates are restricted.</span>
              </div>
            )}
        ...
        {activePage === 'reports' && loggedInUser?.role === 'staff' && (
          <div className="bg-white border border-gray-150 rounded-2xl p-10 text-center max-w-xl mx-auto shadow-md select-none my-6">
            ...
            <h3 className="font-black text-gray-800 text-base uppercase tracking-tight">Access Restricted</h3>
            <p className="text-gray-500 text-xs font-semibold mt-2 leading-relaxed">
              Your current account status as <span className="text-plum font-bold">OPERATIONS STAFF</span> prevents access to Business Intelligence and sales summaries.
            </p>
            <p className="text-gray-400 text-[10px] mt-3 font-semibold uppercase tracking-wider">Rights needed: Admin or Superadmin</p>
          </div>
        )}
        ...
        {activePage === 'analytics' && loggedInUser?.role === 'staff' && (
          <div className="bg-white border border-gray-150 rounded-2xl p-10 text-center max-w-xl mx-auto shadow-md select-none my-6">
            <div className="w-16 h-16 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center mx-auto mb-4 border border-amber-100">
              <ShieldCheck className="w-9 h-9" />
            </div>
            <h3 className="font-black text-gray-800 text-base uppercase tracking-tight">Access Restricted</h3>
            <p className="text-gray-500 text-xs font-semibold mt-2 leading-relaxed">
              Your current account status as <span className="text-plum font-bold">OPERATIONS STAFF</span> prevents access to the 30-Day BI charts & analytics datasets.
            </p>
            <p className="text-gray-400 text-[10px] mt-3 font-semibold uppercase tracking-wider">Rights needed: Admin or Superadmin</p>
          </div>
        )}
        ...
        {activePage === 'settings' && (
          <div className="space-y-6">
            {loggedInUser?.role !== 'superadmin' && (
              <div className="bg-amber-50 border border-amber-200/50 rounded-xl p-4 flex items-start gap-3 text-amber-800 max-w-2xl select-none">
                <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-extrabold text-xs text-amber-900 uppercase tracking-wide">View-Only Operator Session</h4>
                  <p className="text-xs font-semibold mt-1">
                    Your current account status does not permit Store Settings adjustments. Only <span className="font-bold underline">Superadmin</span> can commit parameter modifications.
                  </p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSettingsSubmit} className="space-y-4 text-xs">
              <fieldset disabled={loggedInUser?.role !== 'superadmin'} className="space-y-4">
              ...
              {loggedInUser?.role === 'superadmin' ? (
                ...
                <div className="text-xs text-amber-600 font-extrabold flex items-center gap-2 mt-6 select-none bg-amber-50 border border-amber-200/40 p-3 rounded-lg w-fit">
                  <ShieldCheck className="w-4 h-4 text-amber-500" />
                  <span>Store parameter saving is restricted to Superadmin</span>
                </div>
              )}
            </form>
          </div>
        ...
        {activePage === 'users' && (
          loggedInUser?.role !== 'superadmin' ? (
            <div className="bg-white border border-gray-150 rounded-2xl p-10 text-center max-w-xl mx-auto shadow-md select-none my-6">
              ...
              <h3 className="font-black text-gray-800 text-base uppercase tracking-tight">Access Prohibited</h3>
              <p className="text-gray-500 text-xs font-semibold mt-2 leading-relaxed">
                Your current role restricts access to security credential controls. Only the primary <span className="text-plum font-bold">SUPERADMIN</span> is authorized to access and modify user credentials.
              </p>
              <p className="text-gray-400 text-[10px] mt-3 font-semibold uppercase tracking-wider">Rights required: Superadmin privileges</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* Left Column: Register New Operator */}
              <div className="bg-white border border-gray-150 rounded-xl p-6 shadow-sm h-fit">
                <h3 className="font-black text-gray-800 text-sm border-b border-gray-100 pb-2 mb-4 flex items-center gap-1.5 uppercase tracking-wide">
                  <UserPlus className="w-4.5 h-4.5 text-plum" />
                  <span>Register Admin User</span>
                </h3>
                <form onSubmit={handleAddUserSubmit} className="space-y-4 text-xs font-semibold">
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Username (ID)</label>
                    <input 
                      type="text" 
                      required
                      placeholder="e.g. kamau_j"
                      value={newUserUsername}
                      onChange={(e) => setNewUserUsername(e.target.value)}
                      className="w-full px-3.5 py-2.5 rounded-lg border border-gray-300 outline-none focus:border-plum text-gray-700 bg-white"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Secret Password</label>
                    <input 
                      type="password" 
                      required
                  ...
                  <div>
                    ...
                    <input 
                      type="email" 
                      required
                      placeholder="e.g. kamau@kipchimatt.co.ke"
                      value={newUserEmail}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-600 mb-1">Authorization Role</label>
                    <select 
                      value={newUserRole}
                      onChange={(e) => setNewUserRole(e.target.value as any)}
                      className="w-full px-3.5 py-2.5 rounded-lg border border-gray-300 outline-none focus:border-plum text-gray-700 bg-white cursor-pointer font-bold"
                    >
                      <option value="staff">Staff (Read-only Operator)</option>
                      <option value="admin">Admin (Catalog & Orders Manager)</option>
                      <option value="superadmin">Superadmin (All Credentials & Settings)</option>
                    </select>
                  </div>
                  
                  <button 
                    type="submit"
                    className="w-full bg-plum hover:bg-plum-dark text-white font-bold py-2.5 rounded-lg transition-colors cursor-pointer shadow-sm text-xs mt-6 flex items-center justify-center gap-1.5 border-0"
                  >
              ...
              {/* Right Column: Active Operator Accounts Directory */}
              <div className="bg-white border border-gray-150 rounded-xl p-6 shadow-sm lg:col-span-2 overflow-hidden">
                <h3 className="font-black text-gray-800 text-sm border-b border-gray-100 pb-2 mb-4 flex items-center justify-between uppercase tracking-wide">
                  <div className="flex items-center gap-1.5">
                    <ShieldCheck className="w-4.5 h-4.5 text-plum" />
                    <span>Operator Directory</span>
                  </div>
                  <span className="text-[10px] bg-plum/5 text-plum font-extrabold px-2.5 py-0.5 rounded-full lowercase">
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="border-b border-gray-150 text-gray-400 font-bold uppercase tracking-wider">
                        <th className="pb-3 text-left">Identity Profile</th>
                        <th className="pb-3 text-left">Role Access</th>
                        <th className="pb-3 text-left">Internal Contact</th>
                        <th className="pb-3 text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {usersList.map((usr) => {
                        const isSelf = usr.username === loggedInUser?.username;
                        return (
                          <tr key={usr.id} className="hover:bg-gray-50/50">
                            <td className="py-3">
                              <span className="font-extrabold text-gray-800 block text-sm">{usr.name}</span>
                              <span className="text-[10px] text-gray-450 font-bold mt-0.5 block font-mono">@{usr.username}</span>
                            </td>
                            <td className="py-3">
                              {usr.role === 'superadmin' ? (
                                <span className="bg-plum/10 text-plum border border-plum/15 text-[9px] font-black uppercase px-2 py-0.5 rounded">
                                  SUPERADMIN
                                </span>
                              ) : usr.role === 'admin' ? (
                                <span className="bg-blue-50 text-blue-600 border border-blue-100 text-[9px] font-black uppercase px-2 py-0.5 rounded">
                                  ADMIN
                                </span>
                              ) : (
                                <span className="bg-gray-100 text-gray-600 border border-gray-150 text-[9px] font-black uppercase px-2 py-0.5 rounded">
                                  STAFF OPERATOR
                                </span>
                              )}
                            </td>
                            <td className="py-3 font-semibold text-gray-650">
                              <span className="block">{usr.email}</span>
                            </td>
                            <td className="py-3 text-right">
                              {isSelf ? (
                                <span className="text-[10px] bg-emerald-50 text-emerald-600 font-bold px-2 py-1 rounded border border-emerald-100 select-none">
                                  Active Session
                                </span>
                              ) : (
                                <button 
                                  onClick={() => handleDeleteUser(usr.id)}
                                  className="text-red-600 bg-red-50 hover:bg-red-100 p-1.5 rounded-lg inline-flex items-center justify-center cursor-pointer transition-colors border-0"
                      ...
                      {usersList.length === 0 && (
                        <tr>
                          <td colSpan={4} className="py-8 text-center text-gray-400 font-bold">
                            No active admin accounts loaded.
                          </td>
                        </tr>
                      )}
                    </tbody>
