...
export default function Storefront({
  ...
  const renderProductCard = (p: Product, showBadge = false) => {
    ...
    return (
      <div 
        key={p.id} 
        className="bg-white dark:bg-white rounded-2xl overflow-hidden border border-gray-150 dark:border-gray-200/60 hover:border-plum/40 hover:shadow-2xl transition-all duration-300 flex flex-col group relative"
      >
        <div 
          onClick={() => onProductClick(p)}
          className="h-44 sm:h-48 bg-white dark:bg-white flex items-center justify-center relative overflow-hidden cursor-pointer"
        >
          <img 
        ...
        <div className="p-4 flex-1 flex flex-col">
          <div className="flex items-center justify-between gap-2 mb-1.5">
            <span className="text-[9px] text-plum bg-plum-fade font-black uppercase tracking-widest truncate px-2.5 py-0.5 rounded-full border border-plum/10">
              {p.brand || 'Kipchimatt'}
            </span>
            {p.rating && (
          ...
          <div className="mt-auto">
            ...
            <button 
              ...
              className={`w-full py-2 rounded-lg font-black text-[10px] sm:text-xs flex items-center justify-center gap-1.5 flex-row flex-nowrap whitespace-nowrap cursor-pointer transition-colors uppercase tracking-wider ${isOutOfStock ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : addedProductId === p.id ? 'bg-plum-light text-white' : 'bg-plum text-white hover:bg-plum-light'}`}
            >
              {addedProductId === p.id ? (
                <>
      ...
      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16">
        ...
        {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4 overflow-x-auto pb-4">
                {Array.from({ length: 5 }).map((_, idx) => (
                  <div key={idx} className="min-w-[210px] bg-white dark:bg-white border border-gray-150 dark:border-gray-200/60 p-4 rounded-2xl space-y-4 animate-pulse">
          ...
          <>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                  <div>
                    <h2 className="text-sm font-black text-plum dark:text-pink-300 flex items-center gap-2">
                      <Sparkles className="w-5 h-5 text-plum fill-plum/20 animate-pulse dark:text-pink-300 dark:fill-pink-300/20" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {getFrequentlyBoughtTogether().map(p => {
                    ...
                    return (
                      <div 
                        key={p.id} 
                        className="bg-white dark:bg-white border border-gray-150 dark:border-gray-200/60 rounded-xl p-3 flex items-center gap-3.5 hover:shadow-lg transition-all"
                      >
                        <div 
                          onClick={() => onProductClick(p)}
                          className="w-14 h-14 bg-white dark:bg-white rounded-lg overflow-hidden flex-shrink-0 cursor-pointer"
                        >
                          <img 
                            src={p.image} 
                            alt={p.name} 
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg font-black text-[10px] flex items-center gap-1 cursor-pointer transition-all ${
                            isOutOfStock 
                              ...
                              : 'bg-plum text-white hover:bg-plum-light hover:scale-105'
                          }`}
                        >
                          {addedProductId === p.id ? (
                            <>
            ...
            {/* Kikapu Chapchap Deals Grid */}
            {deals.length > 0 && (
              <section className="py-8 border-y border-plum/15 my-6 bg-plum-fade p-5 rounded-2xl dark:bg-pink-950/5" id="deals-shelf">
                <div className="mb-6">
                  <div className="flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-plum fill-plum/10" />
                    <h2 className="text-lg font-black text-gray-850 dark:text-gray-100 flex items-center gap-2">
                      <span>Kikapu Chapchap Deals</span>
                      <span className="text-[9px] bg-plum text-white font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider border border-plum/15">
                        Flash Savings
                      </span>
                    </h2>
                  </div>
                  <p className="text-gray-500 text-xs mt-1">
                    Super savings on handpicked fresh groceries, pantry essentials, and seasonal delights with absolute lowest price guarantee.
                  </p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {deals.slice(0, 10).map(p => renderProductCard(p, true))}
                </div>
              </section>
            )}

            {/* Popular Products Grid */}
            <section className="py-8 border-y border-plum/10 my-6 bg-plum-fade p-5 rounded-2xl dark:bg-pink-950/5">
              <div className="mb-6">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-plum fill-plum/10" />
                  <h2 className="text-lg font-black text-gray-850 dark:text-gray-100 flex items-center gap-2">
                ...
                <p className="text-gray-500 text-xs mt-1">
                  The most sought-after products, staple groceries, and fresh items highly rated and loved by our local community.
                </p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            ...
            {/* Fresh Food Grid */}
            {fresh.length > 0 && (
              <section className="py-8 border-y border-plum/15 my-6 bg-plum-fade p-5 rounded-2xl dark:bg-pink-950/5">
                <div className="mb-6">
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-gray-500 text-xs mt-1">
                    Locally sourced organic fruits, premium garden vegetables, farm-fresh greens, and crisp produce delivered daily.
                  </p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {fresh.slice(0, 10).map(p => renderProductCard(p))}
                </div>
              </section>
            )}
            ...
            {beverages.length > 0 && (
              <section className="py-8 border-y border-plum/15 my-6 bg-plum-fade p-5 rounded-2xl dark:bg-pink-950/5">
                <div className="mb-6">
                  ...
                  <p className="text-gray-500 text-xs mt-1">
                    Premium local coffees, imported fine teas, pure fruit juices, carbonated soft drinks, energy boosters, and wellness milk.
                  </p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {beverages.slice(0, 10).map(p => renderProductCard(p))}
                </div>
              </section>
            )}
            ...
            {liquor.length > 0 && (
              <section className="py-8 border-y border-plum/15 my-6 bg-plum-fade p-5 rounded-2xl dark:bg-pink-950/5">
                <div className="mb-6">
                  ...
                  <p className="text-gray-500 text-xs mt-1">
                    Carefully curated fine wines, high-end spirits, single malt whiskeys, craft beers, and specialty cocktails. Please drink responsibly.
                  </p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {liquor.slice(0, 10).map(p => renderProductCard(p))}
                </div>
              </section>
            )}
            ...
            <section className="py-6 border-t border-plum/10 dark:border-pink-950/20 mt-4" id="brands-section">
              <div className="mb-6">
                <h2 className="text-base font-extrabold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                  <Star className="w-5 h-5 text-plum fill-plum/10" />
                  <span>Shop by Brand</span>
                </h2>
                <p className="text-gray-500 text-xs mt-0.5">Explore by selecting your favorite domestic or international brand.</p>
              </div>

              <div className="bg-gradient-to-r from-plum-dark via-plum to-plum-light p-6 rounded-2xl border border-plum/30 shadow-lg relative overflow-hidden">
                ...
                <div className="flex flex-wrap gap-2.5 justify-center md:justify-start relative z-10">
                  {Array.from(
                    ...
                    const isActive = activeSearch === brand;
                    return (
                      <button
                        key={brand}
                        onClick={() => onBrandSelect(isActive ? '' : brand)}
                        className={`font-extrabold text-xs px-5 py-2.5 rounded-full cursor-pointer transition-all shadow-sm active:scale-95 border ${
                          isActive
                            ? 'bg-yellow text-gray-950 border-yellow shadow-md shadow-yellow/20 scale-105'
