...
async function startServer() {
  ...
  let users: any[] = readData(USERS_FILE, defaultUsers);

  // Helper to check role rights
  function checkRole(req: express.Request, allowedRoles: string[]): boolean {
  ...
  app.post("/api/auth/users", (req, res) => {
    try {
      if (!checkRole(req, ["superadmin"])) {
        return res.status(403).json({ error: "Forbidden: Superadmin privilege required" });
      }
      ...
      const newUser = {
        ...
        role: role || "staff",
        name: name || username,
        email: email || `${username}@kipchimatt.co.ke`
      };
      users.push(newUser);
  ...
  app.post("/api/products/bulk", (req, res) => {
    try {
      if (!checkRole(req, ["superadmin", "admin"])) {
        return res.status(403).json({ error: "Forbidden: Admin or Superadmin privilege required" });
      }
  ...
  app.put("/api/products/:id", (req, res) => {
    try {
      ...
      // Securing administrative updates while allowing general customer review posting
      if (req.body && (req.body.name !== undefined || req.body.price !== undefined || req.body.stock !== undefined || req.body.category !== undefined)) {
        if (!checkRole(req, ["superadmin", "admin"])) {
          return res.status(403).json({ error: "Forbidden: Admin or Superadmin privilege required to modify catalog elements" });
        }
      }
      products[idx] = {
        ...products[idx],
  ...
  app.post("/api/settings", (req, res) => {
    try {
      if (!checkRole(req, ["superadmin"])) {
        return res.status(403).json({ error: "Forbidden: Superadmin privilege required to modify store settings" });
      }
      settings = {
        ...settings,
        ...req.body
